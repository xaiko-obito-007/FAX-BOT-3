...
function parseMentions(body = "", md = {}) {
	const mentions = {};
	if (!body || !md) return mentions;

	try {
		const prng = JSON.parse(md?.data?.prng || "");
		if (Array.isArray(prng)) {
			for (const m of prng) {
				if (!m?.i || m.o == null || m.l == null) continue;
	...
	const gb =
		...
		md?.data?.Gb?.asMap?.data;

	if (gb && typeof gb === "object") {
		for (const key of Object.keys(gb)) {
			const d = gb[key]?.asMap?.data;
			if (!d) continue;

			const id = d.id?.asLong;
			const offset = Number(d.offset?.asLong);
			const length = Number(d.length?.asLong);

			if (!id || isNaN(offset) || isNaN(length)) continue;
...
function formatDeltaMessage(m) {
	...
	const mentions = parseMentions(body, md);
	...
	return {
		...
		participantIDs: (m.delta.participants || []).map(String)
	};
}
