			...
			for (const i in clientPayload.deltas) {
				...
				if (delta.deltaMessageReaction && !!ctx.globalOptions.listenEvents) {
					...
					if (delta.deltaMessageReply.repliedToMessage) {
						...
						const rmd = rmsg.messageMetadata;
						
						//Mention block - 2#
						callbackToReturn.messageReply = {
							threadID: (
								rmd.threadKey.threadFbId ||
								rmd.threadKey.otherUserFbId
							).toString(),
							messageID: rmd.messageId,
							senderID: rmd.actorFbId.toString(),
							...
							body: rmsg.body || "",
							args: (rmsg.body || "").trim().split(/\s+/),
							isGroup: !!rmd.threadKey.threadFbId,
							mentions: utils.parseMentions(
								rmsg.body || "",
								rmd
							),
...
module.exports = function (defaultFuncs, api, ctx) {
    ...
    // Use the improved getSeqId function from Solution 1
    getSeqId = function getSeqId() {
        ctx.t_mqttCalled = false;
        
        // First verify we have valid cookies
        if (!ctx.jar || ctx.jar.getCookies('https://www.facebook.com').length === 0) {
            const error = { error: "Not logged in", res: "No cookies found" };
            log.error("getSeqId", error);
            ctx.loggedIn = false;
        ...
        defaultFuncs
            ...
            .catch((err) => {
                ...
                if (utils.getType(err) == "Object" && err.error === "Not logged in") {
                    ...
                    log.info("getSeqId", "Trying alternative method...");
                    getSeqIdAlternative(defaultFuncs, api, ctx, globalCallback);
                }
            });
    };

    function getSeqIdAlternative(defaultFuncs, api, ctx, globalCallback) {
        // Alternative method to get sequence ID
        utils.get('https://www.facebook.com/', ctx.jar, null, ctx.globalOptions)
            .then(utils.saveCookies(ctx.jar))
            .then(function(resData) {
                const html = resData.body;
                // Try to extract sequence ID from page
                const seqMatch = html.match(/"irisSeqID":"([^"]+)"/) || 
                               html.match(/irisSeqID[^"']*["']([^"']+)["']/);
                
                if (seqMatch && seqMatch[1]) {
                    ctx.lastSeqId = seqMatch[1];
                    listenMqtt(defaultFuncs, api, ctx, globalCallback);
                } else {
                    throw { error: "Could not get sequence ID from page" };
                }
            })
            .catch(function(err) {
                log.error("getSeqIdAlternative", err);
                globalCallback(err);
            });
    }
