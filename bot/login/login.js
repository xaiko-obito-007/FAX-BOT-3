...
const login = defaultRequire('arshi-fca');
...
const titles = [
	...
	[
		"G O A T B O T  V 3 @" + currentVersion
	],
	[
		"GOATBOT V3"
	]
...
for (const text of title) {
	const textColor = gradient("#00F5FF", "#FF00FF", "#FFD700")(text);
	centerText(textColor, text.length);
}

console.log('');

const versionBadge = `⚡ v${currentVersion} ⚡`;
const versionStyled = gradient("#FFD700", "#FFA500")(versionBadge);
centerText(versionStyled, versionBadge.length);

console.log('');

let tagline = `Next-Gen Messenger Bot Framework`;
const taglineStyled = gradient("#00F5FF", "#7B68EE")(tagline);
centerText(taglineStyled, tagline.length);
...
async function input(prompt, isPassword = false) {
	...
	return new Promise(resolve => rl.question(gradient("#00F5FF", "#7B68EE")(prompt), ans => {
		rl.close();
		resolve(ans);
	}));
}
...
async function startBot(loginWithEmail) {
    ...
    const header = `
╔══════════════════════════╗
║ ${bold}${brightYellow}>>> START LOGGING IN <<<${reset} ║
╚══════════════════════════╝
    `;
    console.log(header);
    ...
    if ([-1, 0].includes(compareVersion(currentVersion, tooOldVersion))) {
        log.err("VERSION", getText('version', 'tooOldVersion', `${brightYellow}node update${reset}`));
        process.exit();
    }
	/* { CHECK ORIGIN CODE } */
    ...
    // ASCII header
    console.log(`${brightYellow}${bold}
╔═════════════════════════════════════╗
║           ░▒▓ BOT INFO ▓▒░          ║
╚═════════════════════════════════════╝
${reset}`);

    // Info lines with symbols and colors
    console.log(`${cyan}➤ NODE VERSION      :${green} ${process.version}${reset}`);
    console.log(`${cyan}➤ PROJECT VERSION   :${green} ${currentVersion}${reset}`);
    console.log(`${cyan}➤ BOT ID            :${green} ${global.botID} - ${await getName(global.botID)}${reset}`);
    console.log(`${cyan}➤ PREFIX            :${green} ${global.GoatBot.config.prefix}${reset}`);
    console.log(`${cyan}➤ LANGUAGE          :${green} ${global.GoatBot.config.language}${reset}`);
    console.log(`${cyan}➤ BOT NICKNAME      :${green} ${global.GoatBot.config.nickNameBot || "GOAT BOT"}${reset}`);

    // Footer line
    console.log(`${brightYellow}${bold}╚═════════════════════════════════════╝${reset}`);
			// ———————————————————— GBAN ————————————————————— //
			let dataGban;
